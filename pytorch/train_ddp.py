...
from disk_based_dataset import DiskBasedDataset
from transformers import BertTokenizer

parser = argparse.ArgumentParser(description='PyTorch Transformer Language Model')
parser.add_argument('--data', type=str, default='../data/wikitext-103', help='location of the data corpus')
parser.add_argument(
    '--dataset', type=str, default='wt103', choices=['wt103', 'lm1b', 'enwik8', 'text8', 'chunyu'], help='dataset name')
...
parser.add_argument(
    '--init_range', type=float, default=0.1, help='parameters initialized by U(-init_range, init_range)')
parser.add_argument(
    '--emb_init_range', type=float, default=0.01, help='parameters initialized by U(-init_range, init_range)')
...
def get_cached_dataset():
    cached_train_dir = os.path.join(args.data, "cached_train_tensors")
    cached_valid_dir = os.path.join(args.data, "cached_valid_tensors")
    cached_test_dir = os.path.join(args.data, "cached_test_tensors")
    if os.path.exists(cached_train_dir):
        train_dataset = DiskBasedDataset.load(cached_train_dir)
        valid_dataset = DiskBasedDataset.load(cached_valid_dir)
        test_dataset = DiskBasedDataset.load(cached_test_dir)
        logging("Loaded from cached dataset")
        return train_dataset, valid_dataset, test_dataset

    train_dataset = DiskBasedDataset(cached_train_dir)
    valid_dataset = DiskBasedDataset(cached_valid_dir)
    test_dataset = DiskBasedDataset(cached_test_dir)
    train_dir = os.path.join(args.data, "train.txt")
    valid_dir = os.path.join(args.data, "valid.txt")
    test_dir = os.path.join(args.data, "test.txt")
    for line in open(train_dir):
        train_dataset.add(line)
    for line in open(valid_dir):
        valid_dataset.add(line)
    for line in open(test_dir):
        test_dataset.add(line)
    train_dataset.save()
    valid_dataset.save()
    test_dataset.save()
    return train_dataset, valid_dataset, test_dataset
...
train_dataset, valid_dataset, test_dataset = get_cached_dataset()
...
def collate_fn(batch, bptt):
    ext_len = args.ext_len or 0
    batch_t = []
    max_len = 0
    for s in batch:
        s_t = [tokenizer.convert_tokens_to_ids(word) for word in s.split()]
        batch_t.append(s_t)
        if len(s_t) > max_len:
            max_len = len(s_t)
    for s in batch_t:
        s.extend((max_len - len(s)) * [pad_id])
    batch_t = torch.LongTensor(batch_t).t().contiguous().to(device)
    ...
    for i in range(0, max_len, bptt):
        ...
        if not data or not target or not seq_len:
            continue
        yield data, target, seq_len


def train_collate_fn(batch):
    return collate_fn(batch, args.tgt_len)


def eval_collate_fn(batch):
    return collate_fn(batch, args.eval_tgt_len)


world_size = dist.get_world_size()
logging(str(world_size))
...
if args.multi_gpu:
    ...
    if args.gpu0_bsz >= 0:
        para_model = BalancedDataParallel(
            args.gpu0_bsz // args.batch_chunk, model, dim=1, device_ids=device_ids).to(device)
    else:
        #para_model = nn.DataParallel(model, dim=1, device_ids=device_ids).to(device)
        para_model = DDP(model.cuda(), dim=1, device_ids=[args.local_rank], output_device=args.local_rank).to(device)
else:
...
def evaluate(eval_iter):
    ...
    with torch.no_grad():
        for i, batches_iter in enumerate(eval_iter):
            mems = tuple()
            for data, target, seq_len in batches_iter:
...
def train():
    ...
    for batch, batches_iter in enumerate(train_iter):
        mems = tuple()
        for data, target, seq_len in batches_iter:
            model.zero_grad()
            if args.batch_chunk > 1:
            ...
            if train_step % args.eval_interval == 0 and dist.get_rank() == 0:
